...
export function formatPersonalTrainerItem(item) {
  return {
    ...
    ptId:item.pt_id ? item.pt_id : 0,
    user_card_id: item.id?item.id:''
  }
}
...
//我的私课时间
export function formatCoachTime(list){
  return list.map(item=>this.formatCoachTimeItem(item))
}
export function formatCoachTimeItem(item){
  return {
    reserveFlag:item.reserveFlag,
