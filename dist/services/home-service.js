...
// 取消团课
export function uploadCancelMyClass(orderId,type) {
...
//扫码销课
export function uploadClassInfoQr(id,flag){
  return urlencodePostRequest('yp-xcx-class-info-qr',{
    custName: appConfig.custName,
    gym: AuthService.getMemberInfo().gym,
    mem_id: AuthService.getMemberInfo().memId,
    class_id:id,
    type_flag:flag,
  })
}
//销课确认
export function uploadClassHandle(class_id,type_flag,user_card_id){
  return urlencodePostRequest('yp-xcx-class-handle',{
    custName: appConfig.custName,
    gym: AuthService.getMemberInfo().gym,
    mem_id: AuthService.getMemberInfo().memId,
    class_id:class_id,
...
// 提交 登录 潜客会员
export function uploadNewMem(infoDic,userInfo) {
  var newInfo = {};
